...
void eventset(struct myevent_s *ev, int fd, void (*call_back)(int fd,int events,void *arg), void *arg)
{
    ...
    ev->events = 0;
    ev->arg = arg;              // ev->arg 其实就相当于this指针
    ev->status = 0;             // status为0表示还没有在epoll树上
    if(ev->len <= 0) {
...
void eventadd(int efd, int events, struct myevent_s *ev)
{
    ...
    epv.events = events;    // 告诉内核需要监听什么事件
    // 内核会把 epv 里的信息拷贝到内核红黑树的节点上。所以传递给epoll_ctl函数的第四个参数可以是栈变量
...
int main()
{
    ...
    while(1)
    {
        ...
        for(i = 0; i < nfd; i++)
        {
            ...
            // 如果监听的是读事件，并返回的是读事件
            // events[i].events表示的是内核返回的可以执行的事件
            ...
            // 前面这个是系统返回过来的事件，是系统告诉我什么事件好了
            // 后面这个是我想要的事件有没有发生
            // if(events[i].events & ((struct myevent_s *)events[i].data.ptr)->events){
            //     ev->call_back(ev->fd, events[i].events, ev->arg);
            // }
        }
    }
